...
def main(conf_name, gpu):
    ...
    for file_ind, input_file in enumerate(files):
        ...
        if gpu == 'all':
            ...
            # Run EZSSR from command line, open xterm for each run
            os.system("xterm -hold -e " + conf.python_path +
                      " %s/run_EZSSR_single_input.py '%s' '%s' '%s' '%s' '%s' '%s' alias python &"
        ...
        # The other option is just to run sequentially on a chosen GPU.
            ...
            run_EZSSR_single_input.main(input_file, ground_truth_file, kernel_files_str, gpu, conf_name, res_dir)
